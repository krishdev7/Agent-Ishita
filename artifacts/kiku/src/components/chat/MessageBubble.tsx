...
function MarkdownContent({ content }: { content: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        // Strip <pre> wrapper so our code component controls the container
        pre({ children }) {
          return <>{children}</>;
        },
        ...
        p({ children }) {
          return (
            <p style={{ margin: '0 0 8px 0', lineHeight: 1.65 }} className="last:mb-0">
              {children}
            </p>
          );
        },
        ...
        a({ href, children }) {
          return (
            <a
              ...
              style={{ color: 'var(--neon-teal)', textDecoration: 'underline', textDecorationColor: 'var(--neon-teal-dim)' }}
            >
              {children}
            </a>
          );
        ...
        ul({ children }) {
          return (
            <ul style={{ margin: '6px 0 8px 0', paddingLeft: 18, listStyleType: 'disc' }}>
              {children}
            </ul>
          );
        },
        ol({ children }) {
          return (
            <ol style={{ margin: '6px 0 8px 0', paddingLeft: 18, listStyleType: 'decimal' }}>
              {children}
            </ol>
          );
        },
        ...
        h1({ children }) {
          return (
            <h1 style={{ fontSize: '1.2em', fontWeight: 700, marginBottom: 8, color: 'rgba(255,255,255,0.95)' }}>
              {children}
            </h1>
          );
        },
        h2({ children }) {
          return (
            <h2 style={{ fontSize: '1.05em', fontWeight: 600, marginBottom: 6, color: 'rgba(255,255,255,0.9)' }}>
              {children}
            </h2>
          );
        },
        h3({ children }) {
          return (
            <h3 style={{ fontSize: '0.95em', fontWeight: 600, marginBottom: 4, color: 'rgba(255,255,255,0.85)' }}>
              {children}
            </h3>
          );
        },
        blockquote({ children }) {
          return (
            <blockquote
              style={{
                borderLeft: '2px solid var(--neon-teal-dim)',
                margin: '6px 0',
                paddingLeft: 12,
      ...
      >
        ...
        <div
          className={`rounded-2xl px-4 py-3 ${
            isUser ? 'rounded-tr-md user-bubble' : 'rounded-tl-md kiku-bubble'
          }`}
        >
          {!isUser && (
            <p
        ...
        <span
          className="mt-1.5 text-[10px] px-1"
